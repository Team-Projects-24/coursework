...
export default async function handler(
  ...
  try {
    ...
    if (timeframe === "month") {
      ...
      >`select userId as "id", date_format(date,"%m-%Y") as "dateFormatted", sum(PerformanceLog.manHoursCompleted) as "manHoursCompleted" from Task inner join PerformanceLog on Task.taskId = PerformanceLog.taskId group by userId, dateFormatted having id in (${Prisma.join(
        userIDs
      )}) order by dateFormatted asc;`;
    } else if (timeframe === "week") {
      results = await prisma.$queryRaw<
        ITimeFramePerformance[]
      >`select userId as "id", concat(date_format(date,"%Y-%m"), " wk ", (week(date,1) - (week(date_format(date,"%Y-%m-01")) - 1))) as "dateFormatted", sum(PerformanceLog.manHoursCompleted) as "manHoursCompleted" from Task inner join PerformanceLog on Task.taskId = PerformanceLog.taskId group by userId, dateFormatted having id in (${Prisma.join(
        userIDs
      )}) order by dateFormatted asc;`;
    } else {
      results = await prisma.$queryRaw<
        ITimeFramePerformance[]
      >`select userId as "id", date_format(date,"%d-%m-%Y") as "dateFormatted", sum(PerformanceLog.manHoursCompleted) as "manHoursCompleted" from Task inner join PerformanceLog on Task.taskId = PerformanceLog.taskId group by userId, dateFormatted having id in (${Prisma.join(
        userIDs
      )}) order by dateFormatted asc;`;
    }
